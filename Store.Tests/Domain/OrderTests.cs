...
namespace Store.Tests.Domain
{
    ...
    {
        private readonly Customer _customer = new Customer("Standilley Oliveira", "standilley@test.io");
        ...
        {
            var order = new Order(_customer, 0, null);
            order.AddItem(_product, 1); // Total deve ser 10, 1 product no valor de 10 reais
            order.Pay(10);
        ...
        public void Dado_um_novo_pedido_valido_seu_total_deve_ser_50()
        {
            ...
            order.AddItem(_product, 5); // 10x5 = 50
        ...
        {
            var expiredDiscount = new Discount(10, DateTime.Now.AddDays(-5)); //discount 0
            var order = new Order(_customer, 10, expiredDiscount);
            order.AddItem(_product, 5); // 10*5 = 50 
            Assert.AreEqual(order.Total(), 60); // 50 + 10(deliveryFee) = 60
        }
        ...
        public void Dado_uma_taxa_de_entrega_de_10_o_valor_do_pedido_deve_ser_60()
        {
            ...
            order.AddItem(_product, 6); // 10x6 = 60
            Assert.AreEqual(order.Total(), 60);
        }
